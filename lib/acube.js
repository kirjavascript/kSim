...
window.__acube__ = function() {

    var config;

    function init(param) {
        config = param;
        config.acube.status = 'idle';
    }

    function solve(str) {

        // check status solving ? 
        if (config.acube.status == 'solving') {
            stop();
            return;
        }
        ...
        var w = creator.createInlineWorker(src, (e) => {
            if (/^Done\./.test(e.data)) {
                config.acube.output.push(e.data);
                stop();
            }
            else if (e.data && e.data[0].trim()
                &&  !/^(Enter|[A-Z]{2} |You can)/.test(e.data)) {
                config.acube.output.push(e.data.replace(/\. /g,''));
            }
        ...
        setTimeout(() => {creator.releaseInlineWorker(w[1])}, 1000);

        [
            config.acube.slices ? 'c' : 'd',
            config.acube.all ? 'a' : '',
            config.acube.optimal ? 'o' : 'p',
            config.acube.metric[0].toLowerCase()
        ].forEach(str => worker.postMessage(["set_opts", str]))
    ...
    function stop() {
        worker.terminate();
        config.acube.status = 'idle';
        config.acube.output.push('Stopped.');
    }
